...
\[
    u = \hat{M}(q)\ddot{q}_r+\hat{C}(q,\dot{q})\dot{q}_r+\hat{D}\dot{q}_r+\hat{ŋ}(q)+K_D\sigma = Y(q,\dot{q},\dot{q}_r,\ddot{q}_r)\hat{\alpha}+K_D\sigma
\]
...
\section{Learning/Repetitive Control}
...
\begin{itemize}
    \item dynamic model of the robot \[
            ;(q)\ddot{q}+C(q,\dot{q})\dot{q}+g(q)=u
    \]
...
Control scheme:
\begin{itemize}
    \item at the $i-th$ iteration, the control actino is \[
...
\begin{enumerate}
    ...
    \item  $0<\beta\leq 0.5$
\end{enumerate}
hold, then the sequence converges to $q_d$ from any initial condition
